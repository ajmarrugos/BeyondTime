...
// Helper to convert hex to rgb for the glow animation
...
const RoutineCard = React.memo(React.forwardRef<HTMLDivElement, RoutineCardProps>(({ 
    ...
    const isLive = useMemo(() => {
        ...
        return now.getTime() >= startTime.getTime() && now.getTime() < endTime.getTime();
    }, [currentTime, routine.autoLive, routine.startTime, routine.endTime]);
    ...
    }, [routine.tasks, completedTaskIds]);
    
    useEffect(() => {
        if (isCompleted && isExpanded) {
            onToggleExpand();
        }
    }, [isCompleted, isExpanded, onToggleExpand]);
    ...
    const handleAnimationEnd = () => {
        if (isDeleting) {
            onDeleteAnimationEnd(routine.id);
        }
    };
    ...
    const classes = [
        'w-full h-full p-4 rounded-3xl relative overflow-hidden flex flex-col',
        themeConfig.cardBg,
        'backdrop-blur-xl border',
        'transition-all duration-300',
        isDeleting ? 'routine-card-fly-out' : '',
        isPlaceholder 
            ? 'opacity-30 border-dashed border-white/30' 
            ...
            <div className={`relative z-10 transition-opacity duration-300 flex-grow flex flex-col ${isCompleted ? 'opacity-60' : 'opacity-100'}`}>
                ...
                <div className="flex items-start space-x-3">
                    <div className={`w-11 h-11 rounded-xl flex-shrink-0 flex items-center justify-center bg-black/20`}>
                        <svg className="w-6 h-6" style={{ color: routine.color }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                            <path strokeLinecap="round" strokeLinejoin="round" d={routine.icon} />
                        </svg>
                    </div>
                        ...
                        {routine.tags.length > 0 && (
                            <div className="flex flex-wrap gap-x-2 gap-y-1 items-center mt-2">
                                {routine.tags.map(tag => (
                                    <div key={tag} className={`py-0.5 px-2.5 rounded-full text-xs font-medium ${themeConfig.subtextColor} border border-white/10 bg-black/10`}>
                                        {tag}
                                    </div>
                                ))}
                            </div>
                ...
                {/* Expandable Section */}
                <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isExpanded ? 'max-h-[60rem] opacity-100' : 'max-h-0 opacity-0'}`}>
                     <div className="border-t border-white/10 pt-4 mt-3 space-y-4">
                        {/* Row 1: Description & Tasks */}
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-2">
                            ...
                            <div>
                                ...
                                <ul className={`space-y-2 text-sm ${themeConfig.subtextColor} max-h-40 overflow-y-auto pr-2`}>
                                    {routine.tasks.length > 0 ? (
                                                ...
                                                <li key={task.id} className="flex items-center space-x-3">
                                                    <button
                                                        ...
                                                        className={`w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-all duration-200 ${
                                                            isTaskCompleted ? 'border-transparent' : 'border-white/30'
                                                        }`}
                                                        style={{ backgroundColor: isTaskCompleted ? routine.color : 'transparent' }}
                                                        aria-label={`Mark task ${task.text} as ${isTaskCompleted ? 'incomplete' : 'complete'}`}
                                                    >
                                                    ...
                                                    {task.budget !== undefined && (
                                                        <span className={`text-xs font-mono px-2 py-0.5 rounded-full bg-black/20 ${isTaskCompleted ? 'opacity-50' : ''}`}>
                                        ...
                                        <li>No tasks for this routine.</li>
                                    )}
                                </ul>
                            </div>
                        </div>
                        ...
                        {totalBudget > 0 && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-white/10 pt-3">
                                <div>
                                    <h4 className={`text-xs font-semibold uppercase tracking-wider mb-1 ${themeConfig.subtextColor}`}>Total Budget</h4>
                                    <p className={`font-mono text-2xl font-bold ${themeConfig.textColor}`}>
                ...
                <footer className={'mt-auto pt-3 border-t border-white/10'}>
                    <div className="flex justify-between items-end">
                        <div className="text-left min-w-0">
                            <p className={`text-sm font-mono truncate ${themeConfig.textColor}`}>{routine.startTime} - {routine.endTime}</p>
                            <p className={`text-xs truncate ${themeConfig.subtextColor}`}>
                                {formatRepetition(routine)}
                            </p>
                        </div>
                        <div className="flex items-center space-x-0.5 flex-shrink-0">
                           <button 
                                ...
                                aria-label={isExpanded ? 'Collapse routine details' : 'Expand routine details'}
                                className={`p-2 rounded-full hover:bg-white/10 transition-colors ${themeConfig.textColor}`}
                            >
                           ...
                           {canEditRoutine(routine) && (
                                <button onClick={handleEdit} aria-label={`Edit ${routine.name}`} className={`p-2 rounded-full hover:bg-white/10 transition-colors ${themeConfig.textColor}`}>
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.5L15.232 5.232z" /></svg>
                                </button>
                            )}
                            {canDeleteRoutine(routine) && (
                                <button onClick={handleDelete} aria-label={`Delete ${routine.name}`} className={`p-2 rounded-full hover:bg-white/10 transition-colors ${themeConfig.textColor}`}>
                            ...
                            <div
                                ...
                                className="self-center p-2 cursor-grab active:cursor-grabbing rounded-full hover:bg-white/10 transition-colors"
                                aria-label="Drag to reorder routine"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 ${themeConfig.subtextColor}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 5v.01M12 12v.01M12 19v.01M5 12h.01M19 12h.01" />
                                </svg>
                            </div>
                        </div>
                    </div>
