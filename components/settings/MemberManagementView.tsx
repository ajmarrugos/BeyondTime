    ...
    return (
        <div className="flex flex-col h-full">
            <div className="flex space-x-2 mb-4">
                <input type="text" value={newMemberName} onChange={(e) => setNewMemberName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAddMemberClick()} placeholder="New member name..." className={`flex-grow p-3 rounded-xl bg-black/20 ${themeConfig.textColor} placeholder-gray-400/70 focus:outline-none focus:ring-2 focus:ring-accent`} />
                <button onClick={handleAddMemberClick} disabled={!newMemberName} className={`px-4 rounded-xl font-semibold ${themeConfig.textColor} bg-white/10 hover:bg-white/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed`}>Add</button>
            </div>
            <ul className="space-y-2 overflow-y-auto flex-1 -mr-2 pr-2">
                {manageableMembers.map(member => {
                    const isPending = !!pendingRoleChanges[member.id];
                    return (
                        <li key={member.id} className={`flex items-center p-3 rounded-xl bg-black/20`}>
