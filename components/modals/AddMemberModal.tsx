...
const AddMemberModal: React.FC<AddMemberModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        addMember({ name, teamId, phone, timezone });
        vibrate();
        onClose();
    };
        ...
        >
            ...
            >
                <div className="flex justify-between items-center mb-4">
                    <h2 className={`text-2xl font-bold ${themeConfig.textColor}`}>Add New Member</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label htmlFor="add-member-name" className={`block mb-1 font-medium ${themeConfig.textColor}`}>Name</label>
                        <input id="add-member-name" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Full name..." className={inputBaseStyle} required />
                    </div>
                     <div className="relative">
                        <label htmlFor="add-member-team" className={`block mb-1 font-medium ${themeConfig.textColor}`}>Team</label>
                        <select id="add-member-team" value={teamId ?? 'unassigned'} onChange={(e) => setTeamId(e.target.value === 'unassigned' ? undefined : Number(e.target.value))} className={`${inputBaseStyle} appearance-none`}>
                     ...
                     <div>
                        <label htmlFor="add-member-phone" className={`block mb-1 font-medium ${themeConfig.textColor}`}>Phone (Optional)</label>
                        <input id="add-member-phone" type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+1-555-0100" className={inputBaseStyle} />
                    </div>
                    <div className="relative">
                        <label htmlFor="add-member-timezone" className={`block mb-1 font-medium ${themeConfig.textColor}`}>Timezone</label>
                        <select id="add-member-timezone" value={timezone} onChange={(e) => setTimezone(e.target.value)} className={`${inputBaseStyle} appearance-none`}>
                    ...
                    <div className="flex justify-end space-x-3 pt-2">
                        ...
                        <button type="submit" className="px-5 py-2.5 rounded-full text-sm font-semibold text-white bg-accent hover:shadow-lg hover:shadow-accent/30 transition-all">Add Member</button>
                    </div>
                </form>
            </div>
        </div>
